...
int symbolCount = 0;
int main(){
    char recordType[20], controlSectionName[20];
    int programLoadAddress, controlSectionAddress, address, length;
    FILE *inputFile, *outputFile;
    inputFile = fopen("INPUT", "r");
    outputFile = fopen("ESTAB", "w");
    printf("Enter the starting address : ");
    scanf("%X", &programLoadAddress);
    controlSectionAddress = programLoadAddress;
    ...
    while(strcmp(recordType, "END") != 0){
        if(strcmp(recordType, "H") == 0){
            ...
            while(strcmp(recordType, "R") != 0){
                ...
                fscanf(inputFile, "%X", &symbols[symbolCount].length);
                symbols[symbolCount].address = symbols[symbolCount].length + controlSectionAddress;
